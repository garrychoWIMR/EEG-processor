...
HistoryFilename = [EEG.filepath, '/', strjoin([BaseFilename; {'history.json'}], '')];
% -----
% Save Sidecar Files
if isfield(EEG.etc, 'JSON')
    % TODO: make sure all JSON fields adequately reflect the data properties
...
if isfield(EEG.etc, 'history')
    ...
    fprintf(fid, '%s', jsonencode(EEG.etc.history, 'PrettyPrint', true));
    fclose(fid);
end
% ---------------------------------------------------------
