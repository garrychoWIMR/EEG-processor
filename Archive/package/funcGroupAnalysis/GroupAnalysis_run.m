...
for i = 1:size(GLM.input, 1)
    ...
    % Make sure the features are column vectors
    for j = 1:length(Data(i).EEG.features)
        if ~any(size(Data(i).EEG.features(j).data) == 1)
            error('Cannot process multidimensional features')
        elseif isrow(Data(i).EEG.features(j).data)
            Data(i).EEG.features(j).data = ascolumn(Data(i).EEG.features(j).data);
        end
    end
end
% --------------------------------------------------
...
DepVars = struct();
% Find which features were common across all files
if any(arrayfun(@(d) size(d.EEG.features(1).data, 2), Data) > 1)
    keyboard
    fprintf('>> BIDS: **************************************************\n')
    fprintf('>> BIDS: WARNING\n')
    fprintf('>> BIDS: Group-level analysis on data with more than 1 trial is not yet supported.\n')
    fprintf('>> BIDS: The dependent variables will be averaged across trials.\n')
    fprintf('>> BIDS: **************************************************\n')
end
feature_types = {Data(1).EEG.features.type};
feature_labels = {Data(1).EEG.features.label};
labels = cellfun(@(t, b) [t, ' ', b], feature_types, feature_labels, 'UniformOutput', false);
for i = 2:length(Data)
    this_labels = cellfun(@(t, b) [t, ' ', b], {Data(i).EEG.features.type}, {Data(i).EEG.features.label}, 'UniformOutput', false);
    labels = labels(ismember(labels, this_labels));
end
% Extract the dependent variable matrix for each feature
for i = 1:length(labels)
    DepVars(i).label = labels{i};
    tmp = arrayfun(@(d) mean(d.EEG.features(i).data, 2), Data, 'UniformOutput', false);
...
badcon = false(length(Tcons),1);
for c = 1:length(Tcons)
    [Xgutt,Zgutt] = palm_partition(DesMat, Tcons(c).Mat', 'Guttman');
    if size(Zgutt, 2) > 0
        cc = palm_cca(Xgutt, Zgutt, 1);
        if cc(1) == 1
            badcon(c) = sum(cc == 1);
        end
    end
...
% --------------------------------------------------
doabort = false; % Set to true if you want to remove the EV and contrast and run the model.
if any(badcon) && ~doabort
    badcon = find(badcon);
    badf = F(badcon, :) > 0;
    badevs = false(1, size(DesMat, 2));
    for c = 1:length(badcon)
        badevs(Tcons(badcon(c)).Mat ~= 0) = true;
    end
    % Remove the bad EVs
    fprintf(2, 'Removing EV %i from design matrix.\n', find(badevs))
    DesMat(:, badevs) = [];
    nmain = length(GLM.model.Predictors);
    nintx = length(GLM.model.Interactions);
    nrand = length(GLM.model.RandVars);
    badmain = badevs(1:nmain);
    badintx = badevs(nmain+1:nmain+nintx);
    badrand = badevs(nmain+nintx+1:nmain+nintx+nrand);
    GLM.model.PredNames(badevs) = [];
    GLM.model.Predictors(badmain) = [];
    GLM.model.Interactions(badintx) = [];
    GLM.model.RandVars(badrand) = [];
    GLM.model.ModelSpec = 'Some predictors and contrasts were removed. Check design matrix.';
    % Remove the bad contrast
    if ~isempty(F) && any(badf)
        fprintf(2, 'Removing contrast %i from F-contrasts.\n', find(badf))
        F(:, badf) = [];
        F(badcon, :) = [];
        GLM.stats.Ftests = F;
    end
    fprintf(2, 'Removing contrast %i from t-contrasts.\n', badcon)
    Tcons(badcon) = [];
    for c = 1:length(Tcons)
        Tcons(c).Mat(badevs) = [];
    end
    T_FTESTS(badcon, :) = [];
    T_FTESTS(:, badevs) = [];
    GLM.stats.Ttests.Titles = {Tcons.Title};
    GLM.stats.Ttests.Sided = {Tcons.Sided};
    GLM.stats.Ttests.Contrasts = cat(1, Tcons.Mat);
elseif any(badcon)
    error('At least one regressor was a linear combination of other regressors (design matrix is rank-deficient).');
end
% --------------------------------------------------
...
% --------------------------------------------------
% Saving and verbose
params.cmcx = '-cmcx '; % run N-permumtations even if the max-num permutations is smaller
