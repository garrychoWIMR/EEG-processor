...
if ~isfield(Import, 'SourceFileType')
    error('Must specify ''SourceFileType'' in the import settings. Choose from ''MFF'', ''COMPU257'', or ''GRAEL''.')
end
if exist(Import.SourceFilePath) == 0 %#ok<EXIST>
    error('Data file not found:\n''%s''.', Import.SourceFilePath)
end
...
if ~isfield(Import, 'TargetFileType')
    Import.TargetFileType = 256;
end
if ~isfield(Import, 'TargetFilePath')
    Import.TargetFilePath = '.';
elseif isempty(Import.TargetFilePath)
    Import.TargetFilePath = '.';
end
ResampleOnImport = false;
if isfield(Import.Processing, 'ResampleOnImport')
    if strcmpi(Import.SourceFileType, 'compu257')
        if Import.Processing.ResampleOnImport
            ResampleOnImport = true;
        end
    end
end
% -------------------------------------------------------------------------
...
[EEG, MFF] = Import_EmptySet(Import.SourceFileType, Import.SourceFilePath, 'DoResample', ResampleOnImport);
% -------------------------------------------------------------------------
% SETTING THE SUBJECT ID
KeyVals = filename2struct(reverse_fileparts(Import.TargetFilePath));
EEG.subject = KeyVals.sub;
% -------------------------------------------------------------------------
% IMPORTING EEG CHANNEL LOCATIONS
[EEG, urpibchanlocs, urpibdata] = Import_ChannelLocations(EEG, Import.Channels.Type, Import.Channels.Path, Import.SourceFileType);
% -------------------------------------------------------------------------
% ONLY KEEP THOSE EEG CHANNELS THE USER SPECIFIED TO LOAD
if Import.TargetFileType == 12
    EEG = Import_SelectTenTwentyChans(EEG, Import.SourceFileType);
end
% -------------------------------------------------------------------------
% LOAD THE EEG DATA
fprintf('>> BIDS: Loading EEG data from file ''%s''.\n', reverse_fileparts(Import.SourceFilePath))
...
switch Import.SourceFileType
    case 'MFF'
        ...
        EEG = mff_import_piblocs(EEG, Import.SourceFilePath);
    otherwise
...
switch Import.SourceFileType
    case 'MFF'
        ...
        T = now;
        EEG = mff_import_events(EEG, Import.SourceFilePath);
...
if Import.TargetFileType ~= 12
    EEG.etc.faster = Proc_FasterStats(EEG, ifelse(strcmpi(Import.SourceFileType, 'TENTWENTY'), true, false));
end
% -------------------------------------------------------------------------
...
[EEG.filepath, EEG.setname] = fileparts(Import.TargetFilePath);
if Import.TargetFileType == 12
...
switch Import.SourceFileType
    case 'MFF'
        EEG.etc.JSON.Manufacturer = 'Electrical Geodesics, Inc.';
        EEG.etc.JSON.ManufacturersModelName = 'NetAmps 400';
        EEG.etc.JSON.SoftwareVersions = 'NetStation Acquisitoin v5.4.2 (r29917)';
    case 'COMPU257'
        EEG.etc.JSON.Manufacturer = 'Compumedics Ltd.';
        EEG.etc.JSON.ManufacturersModelName = 'Neuvo 256';
        EEG.etc.JSON.SoftwareVersions = 'Profusion Sleep Software v5';
    case 'GRAEL'
        EEG.etc.JSON.Manufacturer = 'Compumedics Ltd.';
        EEG.etc.JSON.ManufacturersModelName = 'Grael';
        EEG.etc.JSON.SoftwareVersions = 'Profusion Sleep Software v5';
    case 'REMLOGIC'         % ID: remlogic import GC 13/11/2025
        EEG.etc.JSON.Manufacturer = 'Embla REMlogic';
        EEG.etc.JSON.ManufacturersModelName = 'Titanium';
        EEG.etc.JSON.SoftwareVersions = 'REMlogic v.3.2';

    otherwise
        EEG.etc.JSON.Manufacturer = 'Unknown';
        EEG.etc.JSON.ManufacturersModelName = 'Unknown';
        EEG.etc.JSON.SoftwareVersions = 'Unknown';
end
...
if Import.TargetFileType == 12
    [EEG, ~, Warnings] = SaveTenTwentyDataset(EEG, Import, Warnings);
elseif Import.TargetFileType == 256
    EEG = SaveDataset(EEG, 'all');
end
% -------------------------------------------------------------------------
% Generate the output variable
if Import.TargetFileType ~= -1
