function [ArgOut, Next, Warnings] = ProcPreprocData(Settings)
% ---------------------------------------------------------
% Initialize
ArgOut = [];
Next = 'none';
...
Settings.EEG = LoadDataset(Settings.SourceFilePath, 'all');
% ---------------------------------------------------------
...
if Settings.EEG.trials > 1
    ...
    Settings.EEG.etc.JSON.TrialCount = 1;
end
% ---------------------------------------------------------
...
% ---------------------------------------------------------
% Remove non-cranial channels
if Settings.Reject.NonCranialChannels
    Settings.EEG = Proc_RejectNonCranialChannels(Settings.EEG, Settings.Reject);
end
% ---------------------------------------------------------
...
if Settings.EventSel.Do
    [Settings.EEG, Settings.EventSel, Warnings] = Proc_EpochEventSel(Settings.EEG, Settings.EventSel, Settings.Reject, Settings.TimeSel, Warnings);
end
% ---------------------------------------------------------
...
% Set the filepath
for i = 1:length(Settings.EEG)
    KeyVals = filename2struct(Settings.EEG(i).setname);
    Settings.EEG(i).subject = KeyVals.sub;
    % For legacy support, check if the 'EEG-select' directory exists, otherwise use 'EEG-preproc'
    if exist([Settings.ProtocolPath, '/derivatives/EEG-select'], 'dir') == 0
        if isfield(KeyVals, 'ses')
            Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-preproc/sub-', KeyVals.sub, '/ses-', KeyVals.ses];
        else
            Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-preproc/sub-', KeyVals.sub];
        end
    else
        if isfield(KeyVals, 'ses')
            Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-select/sub-', KeyVals.sub, '/ses-', KeyVals.ses];
        else
            Settings.EEG(i).filepath = [Settings.ProtocolPath, '/derivatives/EEG-select/sub-', KeyVals.sub];
        end
    end
end
% ---------------------------------------------------------
% Set set and filenames
if Settings.EventSel.Do && Settings.EventSel.SeparateFiles
    for i = 1:length(Settings.EventSel.Labels)
        tmp = KeyVals;
        tmp.desc = lower(Settings.EventSel.Labels{i});
        Settings.EEG(i).setname = struct2filename(tmp);
        Settings.EEG(i).filename = [Settings.EEG(i).setname, '.set'];
        Settings.EEG(i).datfile = [Settings.EEG(i).setname, '.fdt'];
    end
else
    for i = 1:length(Settings.EEG)
        Settings.EEG(i).setname = Settings.Setname;
        Settings.EEG(i).filename = [Settings.Setname, '.set'];
        Settings.EEG(i).datfile = [Settings.Setname, '.fdt'];
    end
end
% ---------------------------------------------------------
% Store source file
for i = 1:length(Settings.EEG)
    Settings.EEG(i).etc.JSON.Sources = fullpath2bidsuri(Settings.ProtocolPath, Settings.SourceFilePath);
end
% ---------------------------------------------------------
% Store history
for i = 1:length(Settings.EEG)
    Settings.TargetFilePath = fullfile(Settings.EEG(i).filepath, Settings.EEG(i).filename);
    Settings.EEG(i) = storeHistory(Settings.EEG(i), 'ProcPreprocData', Settings);
end
% ---------------------------------------------------------
% Save the dataset
for i = 1:length(Settings.EEG)
