...
% ID #0021
if isempty(EEG.event) || isempty(fieldnames(EEG.event))
...
for i = 1:length(EEG.event)
    ...
    if isempty(EEG.event(i).id)
        EEG.event(i).id = max([EEG.event.id])+1;
    end
end
if isfield(EEG.event, 'origlatency')
    for i = 1:length(EEG.event)
        if isempty(EEG.event(i).origlatency)
            EEG.event(i).origlatency = EEG.event(i).latency;
        end
    end
end
end
