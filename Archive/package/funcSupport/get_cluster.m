function C = get_cluster(stat_clust, p_clust, a_thres, surface, chanlocs)
% Init
C = struct([]);
...
for c = 1:max(Clusters)
    if length(unique(p_clust(SigNodes(Clusters == c)))) > 1
        warning('More than one unique p-value for cluster %i.', c)
    end
    C(c).chanlocs = chanlocs(SigNodes(Clusters == c));
    C(c).size = ClustSize(c);
    C(c).stat = mean(stat_clust(SigNodes(Clusters == c)));
    C(c).mass = sum(stat_clust(SigNodes(Clusters == c)));
    C(c).p_clust = max(p_clust(SigNodes(Clusters == c)));
    C(c).issig = C(c).p_clust <= a_thres;
end
if max(Clusters) > 0
    % Order by mass
    [~, idx] = sort([C.mass], 'descend');
    C = C(idx);
end
end