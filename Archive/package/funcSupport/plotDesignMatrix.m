...
rmRows = size(DesMat,1);
rmCols = size(DesMat,2);

if size(DesMat, 1) > 50
    rmRows = 26:size(DesMat, 1)-25;
end
if size(DesMat, 2) > 10
    rmCols = 9:size(DesMat, 2)-2;
end

DesMat(rmRows, :) = [];
DesMat(:, rmCols) = [];

GLM.model.ExchangeabilityBlocks(rmRows) = [];
GLM.model.VarianceGroups(rmRows) = [];
GLM.input(rmRows, :) = [];
GLM.stats.Ttests.Contrasts(:, rmCols) = [];
