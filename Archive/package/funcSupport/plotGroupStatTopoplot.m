...
% TODO: Log bug-fix, edge-case where the COPE was all zeros, so replace
% the first value to a tiny number and topoplot won't complain of the
% CLIM
if length(unique(cope)) == 1
    cope(1) = max(cope) + 1e-6;
end
if length(unique(stat)) == 1
    stat(1) = max(stat) + 1e-6;
end
% --------------------------------------------------
% Init default values
